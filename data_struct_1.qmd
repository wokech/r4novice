...
### Vectors
Discussed in the previous chapter **Cross-Ref**
...
•	list[ ] – returns a list  
•	list[[ ]] – returns an element
•	list[[ ]][ ] – returns a …

Flatten out a single vector using unlist()
...
**NOTE**
Tibbles and their advantages

Collection of vectors / Can create a dataframe manually at 1st
...
When you use t() on a dataframe, check using class() to ensure that you didn’t convert to a matrix
To transpose a dataframe use t() and then as.data.frame()
...
glimpse() / View() / unique() / distinct()
complete.cases() - 
eliminate missing values from a vector, matrix, or data frame
