...
The function ```getwd()``` will show the current working directory and 
```setwd(file_path_name)``` will set the working directory to the file path name specified.
...
“path to file” – the location of the file the user intends to import.
header – either TRUE or FALSE, if TRUE, assumes that the file has a header.
sep – field separation characters which include ``` ```, ```,```, ```;```, or ```\t```.
dec – character for decimal points which include ```.``` or ```,```.
stringsAsFactors – by default, strings are read as factors. This can be set as TRUE or FALSE
...
```library(RSQLite)```
```sql_connect <- dbConnect(RSQLite::SQLite(), “____”)```
...
Missing data is a major challenge in data analysis. Strategies for dealing with this are discussed in *Ch 12 (Cross-Reference)*
