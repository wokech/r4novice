...
2.	Functions that can be applied to vectors
...
length() / sum () / median() …max()…
log() / log2() / log10() / count()
round(x, digits = …) / floor() / leading() / 
cumsum() / quantile() / sd() / IQR()  / sin()
...
range() – min and max
cut() – allocates each element to a bin
pretty() – creates breakpoints nicely
...
table()
prop.table()
dimnames() – get names of columns and rows in tables
names(dimnames()) – set the names of columns and rows in tables
...
table(vector1, vector2) – vector1 will go in rows and vector2 will go into columns
